...
void OpenGl_01_drawTri::initializeGL()
{
	...
	// ´´½¨VBO
	m_GLCore->glGenBuffers(1, &VBOPost);
	m_GLCore->glGenBuffers(1, &VBOColor);
	m_GLCore->glGenBuffers(1, &VBOTexture);

	m_GLCore->glBindVertexArray(m_VAO);


	m_GLCore->glBindBuffer(GL_ARRAY_BUFFER, VBOPost);
	...
	m_GLCore->glBindBuffer(GL_ARRAY_BUFFER, VBOTexture);
